...
const getByCamera = (cameraId: string | number, offset: number = 0, limit: number = 5): Promise<ActivityItem[]> => 
    getData<ActivityItem[]>(`/activities/camera/${cameraId}?offset=${offset}&limit=${limit}`);
...
const create = (data: Partial<Activity>): Promise<ActivityResponse> => 
    postDataJSON<ActivityResponse>('/activities', data, "Activity registered successfully!");

const update = (id: string | number, data: Partial<Activity>): Promise<ActivityResponse> => 
    updateData<ActivityResponse>('/activities/', id.toString(), data, "Activity updated successfully!");
